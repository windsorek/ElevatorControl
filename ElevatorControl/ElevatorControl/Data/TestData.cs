...
namespace ElevatorControl.Data
{
    public class TestData
    {
        ...
        {
            var rng = new Random();
            const int totalFloorsMax = 30;
            const int totalFloorsMin = 4;
            const int lowestFloorMin = -10;
            const int lowestFloorMax = 10;
            elevatorControlContext.Elevators.Add(new Elevator((uint)rng.Next(totalFloorsMin, totalFloorsMax), rng.Next(lowestFloorMin, lowestFloorMax)));
            elevatorControlContext.Elevators.Add(new Elevator((uint)rng.Next(totalFloorsMin, totalFloorsMax), rng.Next(lowestFloorMin, lowestFloorMax)));
            elevatorControlContext.Elevators.Add(new Elevator((uint)rng.Next(totalFloorsMin, totalFloorsMax), rng.Next(lowestFloorMin, lowestFloorMax)));
        ...
        public static void AddTestData(IElevatorService elevatorsService)
        {
            ...
            const int lowestFloorMin = -10;
            const int lowestFloorMax = 10;
            elevatorsService.AddElevator(new Elevator((uint)rng.Next(totalFloorsMin, totalFloorsMax), rng.Next(lowestFloorMin, lowestFloorMax)));
            elevatorsService.AddElevator(new Elevator((uint)rng.Next(totalFloorsMin, totalFloorsMax), rng.Next(lowestFloorMin, lowestFloorMax)));
            elevatorsService.AddElevator(new Elevator((uint)rng.Next(totalFloorsMin, totalFloorsMax), rng.Next(lowestFloorMin, lowestFloorMax)));
